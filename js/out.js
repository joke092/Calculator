...
var DecCalculator = function (_Calculator) {
    ...
    _createClass(DecCalculator, [{
        ...
        value: function updateResult() {
            var root = this.$calculatorDOMElement;
            var $resultNumber = root.children(".group-number").children(".result-bit").find("span");
            for (var i = $resultNumber.length - 1, j = 0; i >= 0; i--, j++) {
        ...
        value: function initEvents() {
            ...
            var plus = $(".dec-calculator .operator-bar").children().eq(0);
            plus.on("click", function () {
                var root = _this2.$calculatorDOMElement;
                var activeElement = root.find('.active');
